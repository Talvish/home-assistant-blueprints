...
variables:
  ...
  is_speaker_media_set: >-
    ...
    {# we consider it set if there is a duration, or if we aren't on the tv but the speaker is playing, or #}
    {# this will cover things like radio stations which typically don't have a duration #}
    {%- if ( duration != None and duration >= 0 ) or ( ( not is_speaker_source_tv ) and is_speaker_playing ) -%}
      {{ true }}
    {%- else -%}
      {{ false }}
    {%- endif -%}
...
# Revisions #
* _2022-11-06_: Improved detecting Xbox being on and handles Sonos playing radio stations better
