...
variables:
  alarm: >-
    ...
    {%- set found_alarm = namespace( found = false, entity_id = "", time = "", delta = timedelta(minutes=720 if ( time_window is undefined or time_window < 30 ) else 1439 if time_window > 1439 else time_window )) -%} 
          ...
          - service: "{{ tts_engine }}"
            data:
              ...
              message: >
                ...
                {% set window_hours = ( time_window / ( 60 ) ) | int -%}
                {% set window_minutes = time_window - ( window_hours * 60 ) -%}
                {# so first the hours #}
                {%- set hour_string = "" -%}
                {%- if window_hours == 1 -%}
                ...
                {%- else -%}
                  "Could not find alarm over the next {{ delta_string }}"
                {%- endif -%}
