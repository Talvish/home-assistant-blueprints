...
fields:
  ...
  min_wait:
    ...
    description:
      ...
      Sonos. Frequently Sonos will report state changes to HA before the state has
      changed which can cause odd volume changes and cut-off messages. Setting this
      value will help with both. Defaults to 0 if not set.
    required: false
    selector:
      number:
        min: 0
